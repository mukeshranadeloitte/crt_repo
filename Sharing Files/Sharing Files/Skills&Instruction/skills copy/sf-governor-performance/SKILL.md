...
- Objects where Trigger + Flow + Process Builder all fire on the same event — execution order is unpredictable and unsupported for cross-automation ordering (High)
- Validation rules that duplicate logic already enforced in Apex — dual enforcement creates maintenance risk if they diverge (Medium)
