...
Flag **mixing Flow and Apex triggers as independent entry points on the same object** as High regardless of density — this violates the "one entry point per object" principle and creates unpredictable execution order.
...
| Classic monolithic handler class (all logic in one `OpportunityTriggerHandler`) with no modular atomic classes                             | Medium — recommend metadata-driven framework for medium/high density |
...
| `Database.executeBatch` or `System.schedule` called directly from trigger context                        | Critical — not designed for trigger invocation; rapidly consumes daily async allocation |
| `System.enqueueJob` called without checking `Limits.getQueueableJobs() < Limits.getLimitQueueableJobs()` | High                                                                                    |
| Queueable called from trigger without detecting async context via `System.isBatch()`                     | High — only 1 job allowed from batch execute; up to 50 from synchronous context         |
| Queueable or @future enqueued per-record in a bulk trigger (200 records = 200 jobs) without batching     | Critical — consumes shared daily async limit (250,000 or 200× user licenses)            |

Async pattern selection order (from preferred to last resort):

1. **Flow async path** — for simple fire-and-forget with low volume
2. **Change Data Capture (CDC)** — for high-throughput, durable, decoupled async (72h replay; default 5-object limit)
3. **Queueable Apex** — only when CDC is not viable and Apex control is needed; requires limit guards
4. **Scheduled Job pattern** — for high-volume where CDC object limit is reached: set a status field in the trigger, query-and-process via scheduled job

Flag absence of CDC when the object has high daily DML volume and complex async logic — flag as High.
...
| Single mega-flow containing all automation for an object                                                                      | Medium — split into multiple well-conditioned flows; use Flow Trigger Explorer for order |
| After-save flow used for same-record field updates where a before-save flow or before-trigger would suffice                   | Medium — before-save avoids a second DML and prevents re-firing full save order          |
...
| Flow entry condition does not compare `$Record` vs `$RecordPrior` for field-change-specific logic                             | Medium — risks running on every save even when the relevant field did not change         |
...
When active Process Builders are detected, assess migration complexity and flag the most impactful risks.

| Check | Severity |
| ----- | -------- |
| Active Process Builder on an object that already has a record-triggered Flow — dual execution risk from overlapping automation | High |
| Process Builder containing Apex invocations — migration to Flow requires equivalent Invocable Actions | Medium |
| More than 3 active Process Builders on the same object — migration sequencing required to avoid automation gaps | Medium |
| No documented migration roadmap for active Process Builders despite platform retirement | High |
| Process Builder with cross-object field updates — equivalent Get Records + Update Records elements required in Flow | Low (migration complexity note) |
