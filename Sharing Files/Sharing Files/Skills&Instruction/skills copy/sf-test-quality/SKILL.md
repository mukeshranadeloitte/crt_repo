...
| Multi-object trigger chain (e.g. Opportunity update → Quote trigger → Order creation) has no test verifying end-to-end record state after `Test.stopTest()` | High |
...
| Screen Flow or Experience Cloud page has no functional test plan documented (manual or automated) for the golden path | Medium |
| External system integration has no contract test or schema validation step documented — schema drift goes undetected | Medium |
